...
public class MapWithData {
	...
	//public MapWithData(CityMap map, long agentPlacementRandomSeed) {
		this.map = map;
	...
	public long createMapWithData(Simulator simulator) {
		...
		//CSVNewYorkParser parser = new CSVNewYorkParser(zoneId);
		...
		try {
            for (Resource resource : resourcesParsed) {
				...
				double tripDistane = resource.getTripDistance();
				double fairAmount = resource.getFairAmount();

				ResourceEvent ev = new ResourceEvent(pickupMatch, dropoffMatch, resource.getTime(), simulator, tripDistane, fairAmount);
				events.add(ev);
