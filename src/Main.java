...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		try {
            ...
            String assignType = prop.getProperty("comset.assignmenttype").trim();
            long assignmentType = -1;
            if (assignType != null) {
                assignmentType = Long.parseLong(assignType);
            } else {
                System.out.println("Please enter valid assignment type");
                System.exit(1);
            }

            String hubscount = prop.getProperty("comset.hubs_count").trim();
            long numberOfHubs = -1;

            if (hubscount != null) {
                numberOfHubs = Long.parseLong(hubscount);
            } else {
                System.out.println("Please enter valid hubs count");
                System.exit(1);
            }

            String window = prop.getProperty("comset.windowsize").trim();
            long windowsize = -1;

            if (window != null) {
                if (assignType.equals("2"))
                    windowsize = Long.parseLong(window);
                else
                    windowsize = Long.parseLong(window)/30;
            } else {
                System.out.println("Please enter valid window size");
                System.exit(1);
            }
			...
			simulator.run(assignmentType, numberOfHubs, windowsize);
