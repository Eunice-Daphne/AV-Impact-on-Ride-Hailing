...
public class ResourceEvent extends Event {
	...
	public final static int RESOURCE_AVAILABLE_FOR_PICKUP = 2;
	public final static int RESOURCE_EXPIRED = 3;
	 ...
	 */
	public ResourceEvent(LocationOnRoad pickupLoc, LocationOnRoad dropoffLoc, long availableTime, Simulator simulator, double tripDistance, double fairAmount) {
		super(availableTime, simulator);
		this.pickupLoc = pickupLoc;
		this.dropoffLoc = dropoffLoc;
		this.availableTime = availableTime;
		this.eventCause = RESOURCE_AVAILABLE_FOR_PICKUP;
	...
	Event trigger() throws Exception {
		...
		simulator.polledResourceEvents.add(this.id);
		...
		if (eventCause == RESOURCE_AVAILABLE_FOR_PICKUP) {
			updateWaitingResourcesHandler();
			return null;
		}
		 else {
			expireHandler();
		...
		public void updateWaitingResourcesHandler() {
			Logger.getLogger(this.getClass().getName()).log(Level.INFO," *** Inside updateWaitingResources Handler *** ");
	...
	public Event becomeAvailableHandler() {
		...
		if (earliest > availableTime + simulator.ResourceMaximumLifeTime) {
			...
			this.eventCause = RESOURCE_EXPIRED;
	...
	public void expireHandler() {
		Logger.getLogger(this.getClass().getName()).log(Level.INFO," *** Inside expireHandler *** ");
